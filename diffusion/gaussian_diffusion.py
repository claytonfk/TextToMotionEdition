# This code is based on https://github.com/openai/guided-diffusion and MDM https://github.com/GuyTevet/motion-diffusion-model/
"""
...
class GaussianDiffusion:
    ...
    def weighted_l2(self, a, b, weight):
        # assuming a.shape == b.shape == bs, J, Jdim, seqlen
        # assuming mask.shape == bs, 1, 1, seqlen
        loss = self.l2_loss(a, b)
        loss = sum_flat(loss * weight)  # gives \sigma_euclidean over unmasked elements
        n_entries = a.shape[1] * a.shape[2]
        non_zero_elements = sum_flat(weight) * n_entries
        # print('mask', mask.shape)
        # print('non_zero_elements', non_zero_elements)
        # print('loss', loss)
        mse_loss_val = loss / non_zero_elements
        ...
        # if move:
        #     mse_loss_val += 0.000001/torch.mean((b - torch.mean(b, -1)[..., None]) ** 2)
        # if smooth:
        #     c = b[..., 1:] - b[..., :-1]
        #     c_abs = torch.abs(c)
        #     gt_amp = torch.abs(torch.max(a) - torch.min(a))
        #     prev_amp = torch.abs(torch.max(b) - torch.min(b))
        #
        #     smooth_loss = 0.001*torch.abs(torch.max(c) - mean_flat(c))
        #     smooth_loss += 0.001*torch.abs(prev_amp-gt_amp)
        #     mse_loss_val += smooth_loss
        # print('mse_loss_val', mse_loss_val)
        return mse_loss_val
    ...
    def p_mean_variance(
        ...
        if 'inpainting_mask' in model_kwargs['y'].keys() and 'inpainted_motion' in model_kwargs['y'].keys():
            ...
            assert model_output.shape == inpainting_mask.shape == inpainted_motion.shape, print(model_output.shape, inpainting_mask.shape, inpainted_motion.shape)
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                # if i > 1:
                #     for resample_id in range(0, 20):
                #         new_img = out["sample"] #*np.sqrt(1-self.betas[i]) + th.randn(*shape, device=device)*self.betas[i]
                #         out = sample_fn(
                #             model,
                #             new_img,
                #             t,
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset_name='None'):
        """
        ...
        if 'weight' in model_kwargs['y']:
            weight = model_kwargs['y']['weight']
            use_weight = True
        else:
            use_weight = False
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            # if 'eval_wrapper' in model_kwargs['y']:
            #     m_lens = [target.shape[-1]] * target.shape[0]
            #     m_lens = torch.tensor(m_lens)
            #     target_emb =  target.squeeze(2)
            #     model_output_emb = model_output.squeeze(2)
            #     target_emb = target_emb.permute((0, 2, 1))
            #     model_output_emb = model_output_emb.permute((0, 2, 1))
            #
            #
            #     target_emb = model_kwargs['y']['eval_wrapper'].movement_encoder(target_emb[..., :-4])
            #     target_emb = model_kwargs['y']['eval_wrapper'].motion_encoder(target_emb, m_lens//4)
            #     model_output_emb = model_kwargs['y']['eval_wrapper'].movement_encoder(model_output_emb[..., :-4])
            #     model_output_emb = model_kwargs['y']['eval_wrapper'].motion_encoder(model_output_emb, m_lens//4)
            #
            #     terms["rot_mse"] = mean_flat(self.l2_loss(target_emb, model_output_emb))


            # from utils.soft_dtw_cuda import SoftDTW
            # sdtw = SoftDTW(use_cuda=True, gamma=1)
            # target_w = target*weight
            # model_output_w = model_output*weight
            # target_w = target_w.squeeze(2)
            # model_output_w = model_output_w.squeeze(2)
            # terms["rot_mse"] = sdtw(target_w, model_output_w)

            if use_weight:
                terms["rot_mse"] = self.weighted_l2(target, model_output, weight)
            else:
            ...
            if self.lambda_vel_rcxyz > 0.:
                if self.data_rep == 'rot6d' and dataset_name in ['humanact12', 'uestc']:
            ...
            if self.lambda_vel > 0.:
            ...
            #if True:
                # from data_loaders.humanml.scripts.motion_process import recover_root_rot_pos, quaternion_to_cont6d
                ...
                # checkpoint_folder = checkpoint_path.replace(checkpoint_name, '')
                # opt_path = os.path.join(checkpoint_folder, 'opt.yaml')
                #
                # with open(opt_path, "rb") as optfile:
                #     parameters = yaml.load(optfile, Loader=yaml.Loader)
                #
                # parameters['njoints'] = 22
                # parameters['num_classes'] = 2
                # parameters['nfeats'] = 6
                # device = 'cuda' if torch.cuda.is_available() else 'cpu'
                # motionclip = Encoder_TRANSFORMER(**parameters).to(device)
                # motionclip.eval()
                # state_dict = torch.load(checkpoint_path, map_location=device)
                # missing_keys, unexpected_keys = motionclip.load_state_dict(state_dict, strict=False)
                #
                ...
                #     cont6d_params = cont6d_params.view(-1, cont6d_params.shape[1], joints_num, 6)
                #     #cont6d_params = torch.cat([cont6d_params, r_pos_pad], dim=-2)
                #     return cont6d_params
                #
                # def use_motion_clip(x):
                #     #Shape = [bs, njoints, nfeats, nframes]
                #     batch = {}
                #     n_frames = x.shape[1]
                #     bs = x.shape[0]
                #     batch['x'] = x.permute(0, 2, 3, 1)
                #     batch['y'] = torch.zeros((bs,), dtype=torch.long, device=device)
                #     batch['mask'] = torch.ones((bs, n_frames), dtype=torch.bool, device=device)
                #
                #     out = motionclip(batch)
                #     return out['mu']

                # rot_target = recover_rot(target.transpose(1, 3).squeeze(2))
                # rot_gen = recover_rot(model_output.transpose(1, 3).squeeze(2))
                #
                # #rot_loss = self.l2_loss(rot_target, rot_gen)
                # rot_loss = self.l2_loss(use_motion_clip(rot_target), use_motion_clip(rot_gen))
                # terms["rot_mse2"] = rot_loss


            #print("Mean rot_mse2: ", rot_loss.mean().cpu().detach().numpy())
            #print("Mean rot_mse: ", terms["rot_mse"].mean())
